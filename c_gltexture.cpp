...
/////////////////////////////// CGLTexture class
// CGLTexture::CGLTexture() { Initialize(); pLog     = new CLog("texture.log"); bMadeLog = true; }
// CGLTexture::CGLTexture(string f) { Initialize(); pLog     = new CLog("texture.log"); bMadeLog = true; LoadGL(f); }
// CGLTexture::CGLTexture(CLog *pInLog) { Initialize();if (pInLog) {bMadeLog = false;pLog     = pInLog;} }
// CGLTexture::CGLTexture(CLog *pInLog, string f) { Initialize(); if (pInLog) { bMadeLog = false; pLog     = pInLog; } LoadGL(f); }

CGLTexture::CGLTexture(C_GFX *pInGFX, CGAF *pInGAF, CLog *pInLog, const char *szInFilename) {
    Init();
    pGFX = pInGFX;
    pGAF = pInGAF;
    pLog = pInLog;
...
bool CGLTexture::Load(const char *szInFilename) {
    strcpy(szFilename, szInFilename);
    pTexture = pGFX->LoadGAFTexture(pGAF, szFilename);
}
...
bool CGLTexture::Draw(int x, int y, int x2, int y2, u_char r, u_char g, u_char b, u_char r2, u_char g2, u_char b2) {
    // if (!glBmap) { LoadGL(szFilename); return false; }
    int x3 = (x2 - x);
    int y3 = (y2 - y);
    x      = x / 2;
...
GLuint CGLTexture::LoadGL(const char *szInFilename) {
    LogEntry(va("CWD[%s]\n", dlcs_getcwd()));

    LogEntry(va("CGLTexture::LoadGL(%s) entry \n", szInFilename));
    ...
    ilBindImage(imageID);
    LogEntry(va("Calling iLoadImage(%s) from CGLTexture::LoadGL\n", va("%s%c%s", dlcs_getcwd(), PATH_SEP, &szFilename)));
    success = ilLoadImage(va("%s%c%s", dlcs_getcwd(), PATH_SEP, &szFilename));
    if (success) {
        ILinfo ImageInfo;
