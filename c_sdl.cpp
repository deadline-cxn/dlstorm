...
CSDL_Wrap::CSDL_Wrap()                                                                      { ZeroVars(); SDL_Surface *s; s=0; Begin(0,800,600,32,s); }
CSDL_Wrap::CSDL_Wrap(const char *appname,Uint32 nw,Uint32 nh,Uint32 nb)                     { ZeroVars(); SDL_Surface *s; s=0; Begin(appname,nw,nh,nb,s); }
...
SDL_Surface *CSDL_Wrap::LoadMemSurface(SDL_RWops *rw) {
    ...
    surface=0;
    if(!rw) return 0;
    if(IMG_isBMP(rw)) { surface=IMG_LoadBMP_RW(rw); return surface; }
    if(IMG_isGIF(rw)) { surface=IMG_LoadGIF_RW(rw); return surface; }
    if(IMG_isJPG(rw)) { surface=IMG_LoadJPG_RW(rw); return surface; }
    if(IMG_isLBM(rw)) { surface=IMG_LoadLBM_RW(rw); return surface; }
    if(IMG_isPCX(rw)) { surface=IMG_LoadPCX_RW(rw); return surface; }
    if(IMG_isPNG(rw)) { surface=IMG_LoadPNG_RW(rw); return surface; }
    if(IMG_isPNM(rw)) { surface=IMG_LoadPNM_RW(rw); return surface; }
    if(IMG_isTIF(rw)) { surface=IMG_LoadTIF_RW(rw); return surface; }
    if(IMG_isXCF(rw)) { surface=IMG_LoadXCF_RW(rw); return surface; }
    if(IMG_isXPM(rw)) { surface=IMG_LoadXPM_RW(rw); return surface; }
    if(IMG_isXV(rw))  { surface=IMG_LoadXV_RW(rw);  return surface; }
    if(surface==0)    { surface=IMG_LoadTGA_RW(rw); return surface; }
...
C2DFont::C2DFont(CSDL_Wrap *inSDL) { SDL=inSDL; FontSurface=0; }
C2DFont::C2DFont(CSDL_Wrap *inSDL,const char *file) { SDL=inSDL; FontSurface=0; Load(file); }
...
void CSprite::LoadSurface(const char *fname) {
...
void CSprite::SetColorKey(int r, int g, int b) {
    SDL_SetColorKey(source_surface, SDL_TRUE,SDL_MapRGB(source_surface->format, 0, 0, 0));
}
...
///////////////////////////////////////////////////////////////////////
bool CMouse::MouseIn( int x1,int y1,int x2,int y2) { if((MouseInX(x1,x2))&&(MouseInY(y1,y2))) return true; return false; }
/*
///////////////////////////////////////////////////////////////////////
