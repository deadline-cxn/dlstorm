...
CGameActorAnimation::~CGameActorAnimation() { FramesClear(); }
...
void CGameActorAnimation::FrameAdd(int x, int y, int w, int h) { FrameAdd(pParent->source_surface, x, y, w, h); }
...
CGameActor::~CGameActor() { AnimationsClear(); }
...
void CGameActor::LoadGAFSurface(const char *szFile) { source_surface = pParent->SDL->LoadGAFSurface(szFile); }
