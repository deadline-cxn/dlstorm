...
bool CGAF::AddDir(LPSTR Name){ return AddDir("",Name,1); }

bool CGAF::AddDir(LPSTR Dest, LPSTR dirname, bool SubDirs){
    int n;
	char DirName[GAF_NAMESIZE];
    if(strlen(dirname))
        CabLog->AddEntry(va("Adding Directory:%s",dirname));
    else
        CabLog->AddEntry("WTF!!!");
	if(Dest!=NULL) {
        strcpy(DirName,Dest);
	...
	if(!CreateDir(DirName)) {
        // CabLog->AddEntry(va("Can not create directory:%s",dirname));
        // return false;
	}
