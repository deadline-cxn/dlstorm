...
void  C_GCTRL::get_value(char *str) { strcpy(str, value); }
char *C_GCTRL::get_value(void) { return value; }
void  C_GCTRL::clear_control_data(void) { data.clear(); }
void  C_GCTRL::control_data_unselect(void) { data_selected.clear(); }
void  C_GCTRL::selecta(void) { this->parent_stump->set_group_selecta(this); }
...
void C_GCTRL::sort_control_data(const char method) {
    switch (method) {
        case FM_GSM_ALPHA_ASCEND: break;
        case FM_GSM_ALPHA_DESCEND: break;
        default: break;
    }
}
void C_GCTRL::hide_default_children(void) {
    ...
    switch (type) {
        ...
        case FM_GC_CONSOLE: break;

        default:
            for (i = 0; i < listdepth; i++) {
                if (parent_stump->get_control(va("%s_list_%d", name, i))) parent_stump->get_control(va("%s_list_%d", name, i))->visible = false;
                if (parent_stump->get_control(va("%s_list_image_%d", name, i))) parent_stump->get_control(va("%s_list_image_%d", name, i))->visible = false;
            }
            if (parent_stump->get_control(va("%s_scrollbar", name))) parent_stump->get_control(va("%s_scrollbar", name))->visible = false;
            if (parent_stump->get_control(va("%s_input", name))) parent_stump->get_control(va("%s_input", name))->visible = false;
...
void C_GCTRL::show_default_children(void) {
    ...
    switch (type) {
        ...
        default:
            for (i = 0; i < listdepth; i++) {
                if (parent_stump->get_control(va("%s_list_%d", name, i))) parent_stump->get_control(va("%s_list_%d", name, i))->visible = true;
                if (parent_stump->get_control(va("%s_list_image_%d", name, i))) parent_stump->get_control(va("%s_list_image_%d", name, i))->visible = true;
            }

            if ((int)(data.size()) > listdepth)
                if (parent_stump->get_control(va("%s_scrollbar", name))) parent_stump->get_control(va("%s_scrollbar", name))->visible = true;

            if (parent_stump->get_control(va("%s_input", name))) parent_stump->get_control(va("%s_input", name))->visible = true;
            break;
    }
}
void C_GCTRL::attach_default_children(void) {
    ...
    LogEntry("C_GCTRL::attach_default_children 1\n");
    ...
    if (type == FM_GC_BASE_SELECT) {
        ...
        for (i = 0; i < listdepth; i++) {
            parent_stump->add_control(this, va("%s_list_%d", name, i), FM_GC_STATIC_TEXT, rect.left + 64, rect.top + 16 + i * 32 + 5, 5, 5, 0, "", temp);
            ...
            parent_stump->add_control(this, va("%s_list_image_%d", name, i), FM_GC_STATIC_IMAGE, rect.left + 4, rect.top + 4 + i * 32, 32, 32, 0, "", temp);
    ...
    if (type == FM_GC_LOCAL_DIR) {
        if (pGUI->last_control_clicked) {
            LogEntry(va("[%s][%s][%s][%s][%s]\n", pGUI->last_control_clicked->parent_stump->name, pGUI->last_control_clicked->name, pGUI->last_control_clicked->datafilltarget, pGUI->last_control_clicked->datafill, pGUI->last_control_clicked->action));
        ...
        data.clear();
        for (i = 0; i < dirs.size(); i++) put_control_data("local_dir", (const char *)dirs[i].c_str());
    ...
    if (type == FM_GC_DROPBOX) {
        data_selected.clear();
        parent_stump->add_control(this, va("%s_scrollbar", name), FM_GC_H_SCROLLBAR, rect.left + rect.right + 3, rect.top + 17, 16, ((listdepth - 2) * 16) + 17, 0, "", "");
    ...
    if (type == FM_GC_CONSOLE) {
        LogEntry("C_GCTRL::attach_default_children FM_GC_CONSOLE\n");

        if (pCons) {
            for (i = 0; i < (int)(pCons->buf.size()); i++) put_control_data("listentry", (const char *)pCons->buf[i].c_str());
            LogEntry("ADDING CONSOLE CHILDREN: filling listdepth\n");
            for (i = 0; i < listdepth; i++) {
                strcpy(temp, (const char *)pCons->buf[i].c_str());
                parent_stump->add_control(this, va("%s_list_%d", name, i), FM_GC_STATIC_TEXT, rect.left + 16 + 2, i * 16 + 16,
                                          5,  // rect.right,
                                          5,  // i*16+32,
                                          0, "", temp);
            ...
            LogEntry("ADDING CONSOLE CHILDREN: creating scrollbar\n");

            if (!parent_stump->get_control(va("%s_scrollbar", name))) {
                parent_stump->add_control(this, va("%s_scrollbar", name), FM_GC_H_SCROLLBAR, rect.left, 16, 16, (listdepth - 1) * 16, 0, "", "");
                if (parent_stump->get_control(va("%s_scrollbar", name))) {
            ...
            LogEntry(va("ADDING CONSOLE CHILDREN: creating input (%s)\n", va("%s_input", name)));

            if (!parent_stump->get_control(va("%s_input", name))) {
                parent_stump->add_control(this, va("%s_input", name), FM_GC_TEXTBOX, rect.left, rect.bottom + 1, rect.right, 16, 0, "", "");
            }
            listoffset = control_data_total - listdepth;
        }
        LogEntry("C_GCTRL::attach_default_children 555\n");
    }
...
void C_GCTRL::populate_ctrl_editor(void) {
    ...
    for (ii = pGUI->GC_TYPE.begin(); ii != pGUI->GC_TYPE.end(); ++ii)
        if (((*ii).second) == (type)) strcpy(temp, (const char *)(*ii).first.c_str());
...
char *C_GCTRL::convert$(const char *in) {
    ...
    for (int i = 0; i < (int)(varswap.size()); i++) {
        strcpy(temp, varswap[i].c_str());
        if (temp[0] == '$') {
            for (int j = 0; j < (int)(strlen(temp)); j++) temp[j] = temp[j + 1];
            strcpy(temp3, this->parent_stump->data[temp].c_str());
            LogEntry(va("SWAPPED [%s] to [%s]\n", varswap[i].c_str(), temp3));
            strcat(temp2, temp3);
        } else
    ...
    for (int j = 0; j < (int)(strlen(temp2)); j++) temp2[j] = temp2[j + 1];
    return temp2;
}
void C_GCTRL::checkMouseClicks(bool bMouseWheelUp, bool bMouseWheelDown) {
    ...
    switch (iRelativeTo) {
        case GUI_TOP_CENTER: break;

        case GUI_TOP_RIGHT: break;

        case GUI_MIDDLE_LEFT: break;
        ...
        case GUI_MIDDLE_RIGHT: break;

        case GUI_BOTTOM_LEFT: break;

        case GUI_BOTTOM_CENTER: break;

        case GUI_BOTTOM_RIGHT: break;
    ...
    w = x + rect.right;
    h = y + rect.bottom;
    if (pGUI->bDebug) pGFX->DrawRect(x - 2, y - 2, w + 2, h + 2, dlcsm_RGB(0, 0, pGFX->GetFade(4)));
    if (bMoving) {
        bSizing   = false;
    ...
    if (bSizing) {
        bMoving = false;
        pGFX->DrawRect(x - 1, y - 1, w + 2, h + 2, dlcsm_LONGRGB(155, 155, pGFX->GetFade(2)));
    ...
    if (pGUI->pMouse->In(x, y, w, h)) {
        if (parent_stump->bEditing) {
            pGFX->DrawTransparentBar(x, y, w, h, dlcsm_RGB(255, 255, 0), dlcsm_RGB(255, 255, 0));
        }
    }
    ...
    if (strlen(hover_text)) {
        if (type == FM_GC_DROPBOX)
            if (pGUI->focus_control == this)
                if (pMouse->In(x, y, w, y + (listdepth * 16) + 16)) pGUI->bResetToolTip = true;
    ...
    if (pMouse->RightClick()) {
        if (pMouse->In(x, y, w, h)) populate_ctrl_editor();
    }

    if (pMouse->LeftClick()) {
        LogEntry(va("CHECK MOUSE CLICKS : CLICKED (%s) (%s) (x=%d,y=%d,w=%d,h=%d) GROUP[%s] CONSOLE[%s]\n", tstump->name, name, x, y, w, h, group, console));
        ...
        if (parent_stump != pGUI->focus_stump) {
            ...
            switch (type) {
                case FM_GC_LOCAL_DIR:
                    for (j = 0; j < listdepth; j++) {
                        if (pMouse->In(x, y + (j * 16), w, y + (j * 16) + 16)) {
                            LogEntry(va("%s\n", (const char *)data[j + listoffset].c_str()));
                            if (strlen(datafill)) {
                    ...
                    if (pGUI->focus_control == this) {
                        for (j = 0; j < listdepth; j++) {
                            if (pMouse->In(x - 1, y + (j * 32) + 4, w + 1, y + (j * 32) + 32 + 4)) {
                                pGFX->DrawBar(x - 1, y + (j * 32) + 4, w + 1, y + (j * 32) + 32 + 4, dlcsm_LONGRGB(234, 155, 25));
                ...
                case FM_GC_ACTION:
                    if (pMouse->In(x, y, w, h)) {
                        pMouse->ClearClicks();
                        LogEntry(va("[%s][%s][%s][%s][%s]\n", parent_stump->name, name, action, datafilltarget, datafill));
                        pGUI->last_control_clicked = this;
                        pGUI->focus_control        = 0;
                        if (console[0] != 0) {
                        ...
                        if (pMouse->In(x, y, w, y + (listdepth * 16) + 16)) {
                            ...
                            while (j < (listdepth + 1)) {
                                if (pMouse->In(x - 1, y + (j * 16), w + 1, y + (j * 16) + 16)) {
                                    if (pGUI->bDebug) pGFX->DrawRect(x - 1, y + (j * 16), w + 1, y + (j * 16) + 16, dlcsm_LONGRGB(234, 155, 25));
                                    data_selected.clear();
                ...
                default: pGUI->last_control_clicked = 0; break;
            }
    }

    if (pMouse->ButtonDownTick(SDL_BUTTON_LEFT)) {
        if (pGUI->bDebug) pGFX->DrawRect(x, y, w, h, dlcsm_LONGRGB(255, 0, 0));
        switch (type) {
            case FM_GC_BASE_SELECT:
                for (j = 0; j < listdepth; j++) {
                    if (pMouse->In(x, y + (j * 32) + 4, w, y + (j * 32) + 32 + 4)) {
                        if (pGUI->bDebug) pGFX->DrawBar(x, y + (j * 32) + 4, w, y + (j * 32) + 32 + 4, dlcsm_LONGRGB(234, 155, 25));
                    }
                }
                break;
            ...
            case FM_GC_V_SCROLLBAR:
                ...
                if (pMouse->In(x, h, x + 16, h + 16)) {
                    if (parent_control) {
                        ...
                        if (parent_control->listoffset > (i - parent_control->listdepth)) parent_control->listoffset = i - parent_control->listdepth;
                    }
                    pGUI->focus_control = this;
                    pGUI->focus_stump   = parent_stump;
                }
    ...
    if (bMouseWheelUp) {
        if (pGUI->focus_control == this) listoffset--;
    }
...
void C_GCTRL::draw(bool bMouseWheelUp, bool bMouseWheelDown) {
    ...
    // LogEntry("C_GCTRL::draw() 2\n");

    if (visible) {
        // LogEntry("C_GCTRL::draw() 3\n");

        for (ii = pGUI->GC_TYPE.begin(); ii != pGUI->GC_TYPE.end(); ++ii)
            if (((*ii).second) == (type)) strcpy(temp, (const char *)(*ii).first.c_str());

        // LogEntry(va("C_GCTRL::draw() visible switch(type) type=[%s]\n", temp));

        switch (type) {
            case FM_GC_CHATBOX:
                if (!parent_stump->get_control(va("%s_list_0", name))) attach_default_children();
                if (pGUI->pMouse->InRect(rect))
                    colr = dlcsm_RGB(255, 255, 155);
                else
                    colr = dlcsm_RGB(155, 155, 55);
                pGFX->DrawBar(x, y, w, h, dlcsm_RGB(0, 0, 0), dlcsm_RGB(0, 0, 0));
            ...
            case FM_GC_CONSOLE:
                ...
                if (props & FM_GP_USEBACKGROUND)
                    pGFX->DrawBar(x, y, w, h, dlcsm_LONGRGB(0, 0, 0), dlcsm_LONGRGB(0, 0, 50));
                else
            ...
            case FM_GC_V_SCROLLBAR:
                if (props & FM_GP_USEBACKGROUND) {
                } else {                                           // pGFX->DrawBar(x, y, x+15, h, dlcsm_LONGRGB(180,180,180),dlcsm_LONGRGB(180,180,180));
                    pGFX->Draw3DBox(x - 1, y + 17, w - 1, h + 1);  // , dlcsm_LONGRGB(180,180,180),dlcsm_LONGRGB(180,180,180));
                }
                pGUI->gPrint(x, y, "W", 2);
                if (parent_control) {
            ...
            case FM_GC_DROPBOX:
                ...
                if (!parent_stump->get_control(va("%s_scrollbar", name))) attach_default_children();
                pGFX->DrawBar(x, y, w, y + 16, dlcsm_LONGRGB(102, 102, 170), dlcsm_LONGRGB(12, 12, 170));
                ...
                if (pGUI->focus_control != this) {
                    if (pGUI->pMouse->In(x, y, w, h))
                        pGUI->gPrint(w - 15, y, "^6Y", 2);
                    else
                        pGUI->gPrint(w - 15, y, "^1Y", 2);
                }
                if (pGUI->focus_control == this) {
                    if (parent_stump->get_control(va("%s_scrollbar", name))) {
                        pGFX->DrawBar(x - 2, y + 16, w + 2, y + (listdepth * 16) + 18, 0, 0);
                        pGFX->DrawBar(x - 1, y + 17, w + 1, y + (listdepth * 16) + 17, dlcsm_LONGRGB(132, 132, 200), dlcsm_LONGRGB(12, 12, 170));
                        j = 1;
                        while (j < (listdepth + 1)) {
                            ...
                            pGFX->DrawBar(x, y + (j * 16) + 1, w + 1, y + (j * 16) + 16, dlcsm_LONGRGB(r1, g1, b1), dlcsm_LONGRGB(r2, g2, b2));
            ...
            case FM_GC_LOCAL_DIR:
                ...
                if (props & FM_GP_USEBACKGROUND) {
                    ...
                    pGFX->DrawBar(x + 2, y + 2, w - 2, h - 2, dlcsm_LONGRGB(255, 255, 255), dlcsm_LONGRGB(255, 255, 255));
                }
                if (listoffset > (control_data_total - listdepth)) listoffset = control_data_total - listdepth;
                if (listoffset < 0) listoffset = 0;
                for (i = 0; i < listdepth; i++) {
            ...
            case FM_GC_LISTBOX:
                if (props & FM_GP_USEBACKGROUND) {
                    ...
                    pGFX->DrawBar(x, y, w, h, dlcsm_LONGRGB(180, 180, 180), dlcsm_LONGRGB(180, 180, 180));
                }
            ...
            case FM_GC_TICKBOX:
                if (props & FM_GP_USEBACKGROUND) {
                    ...
                    pGFX->DrawBar(x, y, w, h, dlcsm_LONGRGB(240, 240, 240), dlcsm_LONGRGB(240, 240, 240));
                    pGFX->DrawBar(x + 1, y + 1, w, h, dlcsm_LONGRGB(80, 80, 80), dlcsm_LONGRGB(80, 80, 80));
                    pGFX->DrawBar(x + 2, y + 2, w - 2, h - 2, dlcsm_LONGRGB(180, 180, 180), dlcsm_LONGRGB(180, 180, 180));
                }
                if (dlcs_strcasecmp(value, "on")) pGUI->gPrint(x, y, "^+H", 2);
                break;

            case FM_GC_STATIC_TEXT:
                if (props & FM_GP_USEBACKGROUND) pGFX->DrawBar(x, y - 2, x + (CGLFont_StrLen(value)) * 8 + 24, y + 16, background_color, background_color);
                if (props & FM_GP_BORDER) pGFX->DrawRectangle(x, y - 2, x + (CGLFont_StrLen(value)) * 8 + 24, y + 16, border_color);
                if (font == 0) font = 1;
            ...
            case FM_GC_PASSWORD:
                if (pGUI->focus_control == this) {
                    pGFX->DrawBar(x, y, w, h, dlcsm_LONGRGB(0, 0, 0), dlcsm_LONGRGB(0, 0, 0));
                    pGFX->DrawBar(x + 1, y + 1, w - 1, h - 1, dlcsm_LONGRGB(255, 255, 255), dlcsm_LONGRGB(255, 255, 255));
                    strcpy(temp, value);
                    ...
                    if (pGUI->flashCursor(0, 0)) pGUI->gPrint((int)(x + (CGLFont_StrLen(temp) * 8.5)), y, "^1f", 2);
                } else {
                    pGFX->DrawBar(x, y, w, h, dlcsm_LONGRGB(0, 0, 0), dlcsm_LONGRGB(0, 0, 0));
                    pGFX->DrawBar(x + 1, y + 1, w - 1, h - 1, dlcsm_LONGRGB(255, 255, 255), dlcsm_LONGRGB(255, 255, 255));
                    strcpy(temp, value);
                ...
                if (strlen(value)) {
                    ...
                    if (selected) {
                        if (imedia_selected) {
                            ...
                            pGFX->DrawBar(x, y, w, h, dlcsm_RGB(r1, g1, b1), dlcsm_RGB(r1, g1, b1));
                        }
                    } else {
                        if (imedia_unselected) {
                            pGFX->DrawTexture(x, y, w, h, tctrl2->property["media_unselected"].c_str(), 255, 255, 255);
                        } else {
                            pGFX->DrawBar(x, y, w, h, dlcsm_RGB(r2, g2, b2), dlcsm_RGB(r2, g2, b2));
                        }
                    }
            ...
            case FM_GC_SELECTA_PROPS: break;

            case FM_GC_3D_BOX: pGFX->Draw3DBox(x, y, w, h); break;
            ...
            case FM_GC_BASE_SELECT:
                ...
                if (pGUI->focus_control == this) {
                    ...
                    for (i = 0; i < listdepth; i++) {
                        if (parent_stump->get_control((va("%s_list_image_%d", name, i)))) {
                            strcpy(parent_stump->get_control(va("%s_list_image_%d", name, i))->media, (const char *)data[i + listoffset].c_str());
                        }
                        strcpy(temp, (const char *)data[i + listoffset].c_str());
                        if (temp[strlen(temp) - 1] == '\r') temp[strlen(temp) - 1] = 0;
                        if (temp[strlen(temp) - 1] == '\n') temp[strlen(temp) - 1] = 0;
            ...
            default:
                ...
                pGFX->DrawBar(x, y, w, h, dlcsm_RGB(255, 0, 0));
                pGFX->DrawRectangle(x, y, w, h, dlcsm_RGB(0, 0, 0));  // x+32,y+32,dlcsm_RGB(0,0,0));
                pGUI->gPrint(x, y, temp, 1);
                break;
        }
...
void C_GSTMP::set_group_selecta(C_GCTRL *sgc) {
    ...
    while (tgc) {
        if (dlcs_strcasecmp(sgc->group, tgc->group)) tgc->selected = 0;
        tgc = tgc->next;
    }
    sgc->selected = 1;
}
...
void C_GSTMP::add_control(const char *name, int type, int x, int y, int x2, int y2, int props, const char *media, const char *value) { add_control(0, name, type, x, y, x2, y2, props, media, value); }
...
void C_GSTMP::mod_control(const char *name, int type, int x, int y, int x2, int y2, int props, const char *media, const char *value) {
    LogEntry(va("C_GSTMP::mod_control: %s,%d,%d,%d,%d,%d,%d,%s,%s\n", name, type, x, y, x2, y2, props, media, value));
    ...
    ngc->set_value(value);
    ngc->props = props;
    if (type == FM_GC_HIDDEN) ngc->visible = false;
}
C_GCTRL *C_GSTMP::get_control(const char *name) {
    C_GCTRL *childctl;
    gui_control = first_gui_control;
    while (gui_control) {
        if (dlcs_strcasecmp(name, gui_control->name)) return gui_control;
        childctl = gui_control->first_child_control;
        while (childctl) {
            if (dlcs_strcasecmp(name, childctl->name)) return childctl;
...
void C_GSTMP::del_control(const char *name) {
    ...
    if (!pcontrol) return;
    if (pcontrol->next) pcontrol->next->prev = pcontrol->prev;
    if (pcontrol->prev) pcontrol->prev->next = pcontrol->next;
    ...
    while (cg) {
        if (dlcs_strcasecmp(cg->group, grp)) {
            if (cg->type == FM_GC_GROUPTICK) {
                cg->set_value("off");
                LogEntry(va("Control [%s] a member of group [%s] was turned off\n", cg->name, grp));
            }
        }
        cg = cg->next;
    }
...
void C_GSTMP::store_stmp(void) { store_stmp(name); }
void C_GSTMP::store_stmp(const char *filename) {
    LogEntry(va("Storing GUI stump [%s]\n", filename));
    ...
    while (tgc) {
        ...
        if (tgc->background_color) {
            fputs(va("background_color=%d,%d,%d\n", dlcsm_GetRValue(tgc->background_color), dlcsm_GetGValue(tgc->background_color), dlcsm_GetBValue(tgc->background_color)), fp);
        }
        if (tgc->border_color) {
            fputs(va("border_color=%d,%d,%d\n", dlcsm_GetRValue(tgc->border_color), dlcsm_GetGValue(tgc->border_color), dlcsm_GetBValue(tgc->border_color)), fp);
        }

        if (strlen(tgc->nexttabfocus)) fputs(va("nexttabfocus=%s\n", tgc->nexttabfocus), fp);

        if ((tgc->type != FM_GC_CONSOLE) && (tgc->type != FM_GC_BASE_SELECT)) {
            for (i = 0; i < tgc->data.size(); i++) {
                if (strlen((const char *)tgc->data[i].c_str())) fputs(va("value=%s\n", tgc->data[i].c_str()), fp);
            }
        }
        fputs("\n", fp);
        tgc = tgc->next;
...
C_GUI::C_GUI(C_GFX *pInGFX, CGAF *pInGAF, CLog *pInLog) {
    ...
    pGAF = pInGAF;
    pLog = pInLog;
    LogEntry("C_GUI:C_GUI(C_GFX *pInGFX, CGAF *pInGAF, CLog *pInLog)\n");
    Init();
    bGAFLoading = true;
}
C_GUI::~C_GUI() {
    LogEntry("C_GUI:~C_GUI()\n");
...
void C_GUI::Init(void) {
    ...
    bStmpSizing = 0;
    bDebug      = 0;
    SDL_InitSubSystem(SDL_INIT_EVENTS);  // SDL_InitSubSystem(other stuff)
    LogEntry("C_GUI::init()\n");
    GC_TYPE.clear();
    ...
    LogEntry("C_GUI::init() -> GC_TYPE setup\n");
    GC_PROP.clear();
    ...
    LogEntry("C_GUI::init() -> GC_PROP setup\n");
    GC_RELATIVE.clear();
    ...
    LogEntry("C_GUI::init() -> GC_RELATIVE setup\n");

    pCons = new C_CONS(pGAF, pLog);
    LogEntry("C_GUI: Console created\n");
    ButtonTexture = 0;
    B9utton       = 0;
    pMouse        = 0;
    pMouse        = new C_Mouse(pGFX, pGAF, pLog);
    initFonts();
    initButtons();
    //  loadButtons("");
    //  loadFonts();
    ...
    KeyMap[(SDL_Keycode)'`'] = "toggle console";
    KeyMap[(SDL_Keycode)'~'] = "toggle console";
    KeyMap[(SDL_Keycode)'f'] = "toggle fps";
...
void C_GUI::add_stump(const char *name, int x, int y, int x2, int y2, int props, const char *media) {
    ...
    if (stump) {
        ...
        stump->bSizing   = false;
        stump_count++;
        LogEntry(va("stump added [%s] stump count [%d]\n", name, stump_count));
    }
}
...
void C_GUI::del_stump(C_GSTMP *tstump) {
    ...
    if (tstump) {
        ...
        dlcsm_delete(tstump);
        LogEntry(va("%s (stump count [%d])\n", temp, stump_count));
    }
}
void C_GUI::del_stump(const char *name) { del_stump(get_stump(name)); }
...
void C_GUI::call_do_line(const char *line) {
    ...
    vs = dlcs_explode("\n", line);
    if (vs.size()) vin = dlcs_explode("=", vs[0]);
    ...
    if (vin[0] != "") {
        ...
        switch (mode) {
            ...
            case FGL_GUMP:
                ...
                if (dlcs_strcasecmp((const char *)vin[0].c_str(), "name")) {
                    LogEntry(va("C_GUI::call_do_line mode: FGL_GUMP define gump: name:%s\n", vin[1].c_str()));
                ...
                if (stump) {
                    ...
                    if (dlcs_strcasecmp((const char *)vin[0].c_str(), "relativeto")) {
                        ...
                        switch (stump->iRelativeTo) {
                            ...
                            case GUI_MIDDLE_CENTER:
                                stump->x += pGFX->iScreenWidth / 2;
                                stump->y += pGFX->iScreenHeight / 2;
                            ...
                            case GUI_BOTTOM_CENTER:
                                stump->x += pGFX->iScreenWidth / 2;
                                stump->y += pGFX->iScreenHeight - (stump->rect.bottom);
                                stump->x -= stump->rect.right / 2;
                    ...
                    if (dlcs_strcasecmp((const char *)vin[0].c_str(), "x")) {
                        ...
                        if (dlcs_strcasecmp(vsb, "width")) {
                            stump->x = SDL_GetWindowSurface(pGFX->pWindow)->w - 1;
                        }
                        if ((int)vsb[vsbl - 1] == (int)'*') {
                            vsb[vsbl - 1] = 0;
                            stump->x      = SDL_GetWindowSurface(pGFX->pWindow)->w * (atoi(vsb) / 100);
                        } else {
                    ...
                    if (dlcs_strcasecmp((const char *)vin[0].c_str(), "y")) {
                        ...
                        if (dlcs_strcasecmp(vsb, "height")) {
                            stump->y = SDL_GetWindowSurface(pGFX->pWindow)->h;
                        }
                        if ((int)vsb[vsbl - 1] == (int)'*') {
                            vsb[vsbl - 1] = 0;
                            stump->y      = SDL_GetWindowSurface(pGFX->pWindow)->h * (atoi(vsb) / 100);
                        } else {
                    ...
                    if (dlcs_strcasecmp((const char *)vin[0].c_str(), "w")) {
                        ...
                        if ((int)vsb[vsbl - 1] == (int)'*') {
                            vsb[vsbl - 1]     = 0;
                            stump->rect.right = SDL_GetWindowSurface(pGFX->pWindow)->w * (atoi(vsb) / 100);
                        } else {
                            stump->rect.right = atoi(vin[1].c_str());
                        }
                        if (dlcs_strcasecmp(vsb, "width")) {
                            stump->rect.right = SDL_GetWindowSurface(pGFX->pWindow)->w - 1;
                        }
                        return;
                    }
                    ...
                    if (dlcs_strcasecmp((const char *)vin[0].c_str(), "h")) {
                        stump->rect.bottom = 0;
                        if (dlcs_strcasecmp(vsb, "height")) {
                            stump->rect.bottom = SDL_GetWindowSurface(pGFX->pWindow)->h;
                        }
                        if ((int)vsb[vsbl - 1] == (int)'*') {
                            vsb[vsbl - 1]      = 0;
                            stump->rect.bottom = (SDL_GetWindowSurface(pGFX->pWindow)->h * ((float)(atoi(vsb) / 100)));
                        } else {
            ...
            case FGL_CTRL:
                if (stump) {
                    ...
                    if (dlcs_strcasecmp((const char *)vin[0].c_str(), "value")) {
                        if ((ctrl->type == FM_GC_LISTBOX) || (ctrl->type == FM_GC_DROPBOX)) {
                            ctrl->put_control_data("listentry", (const char *)vin[1].c_str());
                            if (((CGLFont_StrLen((const char *)vin[1].c_str()) * 9.5) + 9) > (ctrl->rect.right)) ctrl->rect.right = (CGLFont_StrLen((const char *)vin[1].c_str()) * 9.5) + 9;
                        } else {
                    ...
                    if (dlcs_strcasecmp((const char *)vin[0].c_str(), "border_color")) {
                        ...
                        if (vs.size() > 2) {
                            ...
                            g                  = atoi(vs[1].c_str());
                            b                  = atoi(vs[2].c_str());
                            ctrl->border_color = dlcsm_RGB(r, g, b);
                        } else {
                        }
                        return;
                    }
                    ...
                    if (dlcs_strcasecmp((const char *)vin[0].c_str(), "background_color")) {
                        ...
                        if (vs.size() > 1) {
                            ...
                            g                      = atoi(vs[1].c_str());
                            b                      = atoi(vs[2].c_str());
                            ctrl->background_color = dlcsm_RGB(r, g, b);
                        } else {
                        }
                        return;
                    }
                    ...
                    if (dlcs_strcasecmp((const char *)vin[0].c_str(), "w")) {
                        if (dlcs_strcasecmp((const char *)vin[1].c_str(), "width")) {
                            ctrl->rect.right = SDL_GetWindowSurface(pGFX->pWindow)->w - 1;
                        } else {
                    ...
                    if (dlcs_strcasecmp((const char *)vin[0].c_str(), "relativeto")) {
                        ...
                        LogEntry(va("GUI CONTROL: %s relative to %s (%d) (%d)\n", ctrl->name, vin[1].c_str(), ctrl->iRelativeTo, GC_RELATIVE[(const char *)vin[1].c_str()]));
                        return;
                    }
                    ...
                    if (dlcs_strcasecmp((const char *)vin[0].c_str(), "visible")) {
                        ctrl->visible = false;
                        if (dlcs_strcasecmp((const char *)vin[1].c_str(), "yes")) ctrl->visible = true;
                        if (dlcs_strcasecmp((const char *)vin[1].c_str(), "true")) ctrl->visible = true;
                        if (dlcs_strcasecmp((const char *)vin[1].c_str(), "1")) ctrl->visible = true;
                        return;
                    }
...
void C_GUI::cab_call(const char *file) {
    ...
    LogEntry(va(" nfb.Size=%d nfb.fb=%d(address)\n", nfb.Size, nfb.fb));

    if (nfb.Size) {
        for (nfbindex = 0; nfbindex < nfb.Size; nfbindex++) {
            fline[lindex] = nfb.fb[nfbindex];

            if ((nfb.fb[nfbindex] == '\r') || (nfb.fb[nfbindex] == '\n')) {
                if (lindex) {
                    fline[lindex] = 0;
                    lindex        = 0;
                    call_do_line(fline);
    ...
    LogEntry(va("   GUI->call(gumps/%s) end\n", file));
}
void C_GUI::bcall(const char *file) {
    LogEntry(va(" GUI->bcall(gumps/%s)\n", file));
    //	CxMemFile *hi;
    if (!pGAF) return;
    GAF_FileBuffer nfb;
    char           fin[1024];
...
void C_GUI::bstore(const char *file) {
    pGAF->AddFileFromMem((const char *)va("gumps/new_%s", file), (unsigned char *)&first_gui_stump, sizeof(&first_gui_stump));
...
void C_GUI::moveStumpToTop(C_GSTMP *pStump) {
    C_GSTMP *pTempStump;
    if (pStump) {
        if (pStump->prev) pStump->prev->next = pStump->next;

        if (pStump->next) pStump->next->prev = pStump->prev;
        ...
        if (focus_stump == pStump) {
            focus_stump = pStump;
            if (focus_control)
                if (focus_control->parent_stump != pStump) focus_control = 0;
        }
    }
}
bool C_GUI::isAStumpMoving(void) {
    ...
    pTempStump = first_gui_stump;
    while (pTempStump) {
        if (pTempStump->bMoving) return true;
        pTempStump = pTempStump->next;
    }
    return false;
}
void C_GUI::checkMouseClicks(void) {
    ...
    while (tstump) {
        ...
        if (bDebug) pGFX->DrawRect(x - 2, y - 2, w + 2, h + 2, dlcsm_LONGRGB(pGFX->GetFade(4), 234, 225));
                            ...
                            if (!bStmpMoving)
                                if (pMouse->In(w - 32, h - 32, w + 2, h + 2)) {
                                    if (pMouse->ButtonDown(SDL_BUTTON_LEFT)) {
                                        ...
                                        tstump->bMoving = false;
                                        focus_stump     = tstump;
                                        pGFX->DrawRect(w - 32, h - 32, w + 2, h + 2, dlcsm_LONGRGB(155, pGFX->GetFade(1), 255));
...
void C_GUI::draw_ctrls(void) {
    ...
    bool bMouseWheelUp, bMouseWheelDown;
    bMouseWheelUp = bMouseWheelDown = 0;
    ...
    if (focus_control) focus_stump->move_control_to_top(focus_control);
    tstump = first_gui_stump;
    while (tstump) {
        x = tstump->rect.left;
        y = tstump->rect.top;
        ...
        if (tstump->props & FM_GP_FULLSCREEN) {
            tstump->rect.left   = 0;
            tstump->rect.top    = 0;
            tstump->rect.bottom = SDL_GetWindowSurface(pGFX->pWindow)->h;
            tstump->rect.right  = SDL_GetWindowSurface(pGFX->pWindow)->w;
        } else {
        }

        if (tstump->props & FM_GP_MOVEABLE)
            if (tstump->bMoving) {
                pGFX->DrawTransparentBar(tstump->rect.left - 4, tstump->rect.top - 4, tstump->rect.left + tstump->rect.right + 4, tstump->rect.top + tstump->rect.bottom + 4, dlcsm_LONGRGB(240, 240, 240), dlcsm_LONGRGB(240, 240, 240));
            }
        if (tstump->props & FM_GP_SIZEABLE)
            if (tstump->bSizing) {
                pGFX->DrawBar(w - 32, h - 32, w + 2, h + 2, dlcsm_LONGRGB(155, 155, pGFX->GetFade(0)));
            } else {
                if (pMouse->In(w - 32, h - 32, w + 2, h + 2)) {
                    pGFX->DrawBar(w - 32, h - 32, w + 2, h + 2, dlcsm_LONGRGB(155, pGFX->GetFade(1), 255));
                }
            }

        if (tstump->props & FM_GP_USEBACKGROUND) {
            pGFX->DrawTexture(tstump->rect.left, tstump->rect.top, tstump->rect.left + tstump->rect.right, tstump->rect.top + tstump->rect.bottom, tstump->media, 255, 255, 255);
        } else {
        ...
        if (tstump->props & FM_GP_MENU) {
            if (focus_stump == tstump)
                pGFX->DrawBar(tstump->rect.left + 1, tstump->rect.top + 1, tstump->rect.left + tstump->rect.right - 1, tstump->rect.top + 18 - 1, dlcsm_LONGRGB(0, 60, 160), dlcsm_LONGRGB(60, 0, 60));
            else
                pGFX->DrawBar(tstump->rect.left + 1, tstump->rect.top + 1, tstump->rect.left + tstump->rect.right - 1, tstump->rect.top + 18 - 1, dlcsm_LONGRGB(60, 60, 60), dlcsm_LONGRGB(10, 10, 10));
...
void C_GUI::drawToolTip(void) {
    if (bDrawToolTip) {
        pGFX->DrawRect(rectToolTip.left - 1, rectToolTip.top - 1, rectToolTip.right + 1, rectToolTip.bottom + 1, dlcsm_RGB(0, 0, 0));
        pGFX->DrawBar(rectToolTip.left, rectToolTip.top, rectToolTip.right, rectToolTip.bottom, dlcsm_RGB(255, 255, 0));
...
void C_GUI::setdata(const char *stump, const char *ctrl, const char *value) {
    if (get_stump(stump))
        if (get_stump(stump)->get_control(ctrl)) get_stump(stump)->get_control(ctrl)->set_value(value);
}
void C_GUI::edit_stump(const char *file) {
    ...
    if (editstump) {
        ...
        LogEntry(va("con_guiedit: loaded %s\n", file));
    }
...
void C_GUI::drawB9utton(int x, int y, int w, int h) {
    ...
    for (i = 0; i < 9; i++) {
        if (!B9utton[i].texture)
            B9utton[i].texture = new CGLTexture(pGFX, pGAF, pLog, va("buttons/b9_%03d.bmp", i));
        else {
            if (!B9utton[i].texture->glBmap)
                B9utton[i].texture->LoadGL(va("buttons/b9_%03d.bmp", i));
            else {
                cx = x + (col * 8);
                cy = y + (row * 8);
...
void C_GUI::drawGUIButton(int x, int y, int which, int size) {
    ...
    if (!ButtonTexture[which].texture) {
        ButtonTexture[which].texture = new CGLTexture(pGFX, pGAF, pLog, va("buttons/%03d.bmp", which));
        return;
    }
    if (!ButtonTexture[which].texture->glBmap) {
        ButtonTexture[which].texture->LoadGL(va("buttons/%03d.bmp", which));
        return;
    }
    ButtonTexture[which].texture->Draw(x, y, x + size, y + size, 255, 255, 255);
}
void C_GUI::drawGUIResource(int w, int x, int y, int x2, int y2) { drawGUIResourceC(w, x, y, x2, y2, 255, 255, 255); }
void C_GUI::drawGUIResourceC(int which, int iX, int iY, int iX2, int iY2, u_char r, u_char g, u_char b) {
    // draw a GUI resource at x,y
    if (!ButtonTexture) return;
    if (!ButtonTexture[which].texture) {
        ButtonTexture[which].texture       = new CGLTexture(pGFX, pGAF, pLog, va("buttons/%03d.bmp"));
        ButtonTexture[which].texture->pGAF = pGAF;
        return;
    }
    if (!ButtonTexture[which].texture->glBmap) {
        ButtonTexture[which].texture->LoadGL(va("buttons/%03d.bmp"));
        return;
    }
...
void C_GUI::drawCPUSTRING(int iX, int iY) {
    drawOSIcon(iX, iY, DLCS_CPU_STRING);
    gPrint(iX + 10, iY, va("^+^1(^5%s^1)", DLCS_CPU_STRING), 1);
}
...
bool C_GUI::initButtons() {
    ...
    for (i = 0; i < MAX_BUTTONS; i++) ButtonTexture[i].texture = 0;
...
bool C_GUI::destroyButtons() {
    int i;
    for (i = 0; i < 9; i++) dlcsm_delete(B9utton[i].texture);
    delete[] B9utton;
    B9utton = 0;
    for (i = 0; i < MAX_BUTTONS; i++) dlcsm_delete(ButtonTexture[i].texture);
    delete[] ButtonTexture;
    ButtonTexture = 0;
    return true;
}
bool C_GUI::initFonts() {
    LogEntry("Begin Fonts init...\n");
    loadFonts();
    return 1;
}
bool C_GUI::loadFonts(void) {
    LogEntry("Begin Fonts load...\n");
    CGLFont *      pFont;
    CGLFont *      pOldFont = 0;
    int            x        = 1;
    DIR *          dpdf;
        ...
        while (epdf = readdir(dpdf)) {
            if (!((dlcs_strcasecmp(epdf->d_name, ".")) || (dlcs_strcasecmp(epdf->d_name, ".."))) && (!dlcs_isdir(epdf->d_name))) {
                ...
                if (pFont) {
                    ...
                    pFont->pNext = new CGLFont(pGFX, pGAF, pLog);
                    pOldFont     = pFont;
                    pFont        = pFont->pNext;
                } else {
                    pFirstFont = new CGLFont(pGFX, pGAF, pLog);
                    pFont      = pFirstFont;
                }

                char szTempFilename[_FILENAME_SIZE];
                memset(szTempFilename, 0, _FILENAME_SIZE);
                strcpy(szTempFilename, va("fonts/%s", epdf->d_name));
                pFont->Load(szTempFilename);
                pFont->iWhich = x;
                x++;
                if (!pFont->pFontTex) {
                    LogEntry(va("ERROR LOADING %s (CGLTEXTURE OBJECT DESTROYED)\n", szTempFilename));
                    if (pFont == pFirstFont) {
                    ...
                    LogEntry(va("Font texture %s loaded (OPENGL[%d])\n", pFont->pFontTex->szFilename, pFont->pFontTex->glBmap));
                }
            }
        }
    }
...
CGLFont *C_GUI::GetFont(const char *szFont) {
    ...
    pFont = pFirstFont;
    while (pFont) {
        if (dlcs_strcasecmp(pFont->pFontTex->szFilename, szFont)) {
            return pFont;
        }
        pFont = pFont->pNext;
    }
...
void C_GUI::drawFPS(int iX, int iY) {
    ...
    FrameCount++;
    if ((iX == 0) && (iY == 0)) {
        iX = pGFX->iScreenWidth - 80;
        iY = pGFX->iScreenHeight - 32;
    }
...
void C_GUI::drawFileXferStatus(void) {
    ...
    gPrint(10,310,va("Recieving file[%s]",pPlayer->DownloadInfo.szLocalXferName),dlcsm_LONGRGB(0,255,0));
    gPrint(10,320,va("File size[%d] u_chars",pPlayer->DownloadInfo.iFileSize),dlcsm_LONGRGB(0,255,0));
    gPrint(10,330,va("Progress[%d] u_chars",pPlayer->DownloadInfo.iTotalBytesRead),dlcsm_LONGRGB(0,255,0));
    // gPrint(10,340,va("Transfer Rate :%d u_chars per second",pPlayer->iBytesPerSecond),dlcsm_LONGRGB(0,255,0));
...
int C_GUI::DoInput() {
    ...
    return 0;  // processKeyboard(event);
}
int C_GUI::DoKeyboard(SDL_Event event) {
    char temp[1024];
    memset(temp, 0, 1024);
    bool bDone;
    bDone    = 0;
    ...
    switch (event.type) {
        case SDL_KEYUP:
            ...
            if ((ikey >= 'a') && (ikey <= 'z') && (modstate & KMOD_SHIFT)) ikey = (SDL_Keycode)(ikey - 32);

            if (modstate & KMOD_SHIFT) {
                switch (ikey) {
                    case '1': ikey = (SDL_Keycode)'!'; break;
                    case '2': ikey = (SDL_Keycode)'@'; break;
                    case '3': ikey = (SDL_Keycode)'#'; break;
                    case '4': ikey = (SDL_Keycode)'$'; break;
                    case '5': ikey = (SDL_Keycode)'%'; break;
                    case '6': ikey = (SDL_Keycode)'^'; break;
                    case '7': ikey = (SDL_Keycode)'&'; break;
                    case '8': ikey = (SDL_Keycode)'*'; break;
                    case '9': ikey = (SDL_Keycode)'('; break;
                    case '0': ikey = (SDL_Keycode)')'; break;
                    case ',': ikey = (SDL_Keycode)'<'; break;
                    case '.': ikey = (SDL_Keycode)'>'; break;
                    case ';': ikey = (SDL_Keycode)':'; break;
                    case '\'': ikey = (SDL_Keycode)'"'; break;
                    case '/': ikey = (SDL_Keycode)'?'; break;
            ...
            KeyDown[(SDL_Keycode)ikey] = false;
            ...
            if (!focus_control) {
                switch (ikey) {
                    ...
                    case SDLK_UP:
                        ...
                        break;
                    case SDLK_DOWN: pGFX->pCamera->Move_Backward_Stop(); break;
                    case SDLK_LEFT: pGFX->pCamera->Move_Left_Stop(); break;
                    case SDLK_RIGHT: pGFX->pCamera->Move_Right_Stop(); break;

                    case SDLK_PAGEDOWN: break;
                    case SDLK_PAGEUP: break;
                    default: return ikey; break;
                }
            }
            break;
        ...
        case SDL_KEYDOWN:
            ...
            KeyDown[(SDL_Keycode)ikey] = true;
            KeyRepeatTimer             = 0;

            if (focus_control) {
                if ((focus_control->type == FM_GC_TEXTBOX) || (focus_control->type == FM_GC_PASSWORD)) {
                    ...
                    switch (ikey) {
                        ...
                        case 27: focus_control = focus_control->next; break;
            ...
            if (!focus_control) {
                switch (ikey) {
                    case SDLK_UP:
                        ...
                        break;
                    case SDLK_DOWN: pGFX->pCamera->Move_Backward_Start(); break;
                    case SDLK_LEFT: pGFX->pCamera->Move_Left_Start(); break;
                    case SDLK_RIGHT: pGFX->pCamera->Move_Right_Start(); break;
                    case SDLK_PAGEDOWN: break;
                    case SDLK_PAGEUP: break;
                    default: break;
                }
    ...
    if ((dlcs_get_tickcount() - KeyRepeatTimer) > 600) {
        ...
        for (int tkey = 0; tkey < 255; tkey++) {
            if (KeyDown[(SDL_Keycode)tkey] == true) {
                switch (tkey) {
                    case 8:
                    ...
                    default:
                        ...
                        if (focus_control) {
                            focus_control->get_value(temp);
                            if ((tkey >= ' ') && (tkey <= '~') && (CGLFont_StrLen(temp) < focus_control->max_val_len)) {
    ...
    if ((event.type) == SDL_KEYUP) return ikey;
    return 0;
}

int C_GUI::DoMouse(SDL_Event event) {
    static bool bWarped;
    static int  ilmx;
    static int  ilmy;
    ...
    switch (event.type) {
        case SDL_MOUSEBUTTONDOWN:
            ...
            // if(event.button.button==SDL_BUTTON_WHEELUP) {                pMouse->SetWheelUp(1);            }            pMouse->SetWheelDown(0);
            // if(event.button.button==SDL_BUTTON_WHEELDOWN) {                pMouse->SetWheelDown(1);            }
            break;
    }

    if (event.wheel.y > 0) {         // scroll up
        pMouse->SetWheelUp(1);       // bWheelUp = true;
    } else if (event.wheel.y < 0) {  // scroll down
        pMouse->SetWheelDown(1);     // bWheelDown = true;
    }
    ...
    if (pMouse->bRightDown) {
        pMouse->bDraw = 0;
        bWarped       = 0;
        if ((ilmx != pMouse->ix) || (ilmy != pMouse->iy)) {
        ...
        if (!bWarped) {
            bWarped = true;
            SDL_WarpMouseInWindow(pGFX->pWindow, ilmx, ilmy);
        }
    }
}
...
bool C_GUI::bSaveBinds(const char *szFilename) {
    ...
    for (ikey = 0; ikey < SDL_NUM_SCANCODES; ikey++) {
        strcpy(szTemp, "bind ");
        ...
        if (ikey == SDLK_NUMLOCKCLEAR) strcat(szTemp, "numlock ");
        if (ikey == SDLK_SCROLLLOCK) strcat(szTemp, "scroll ");
        if (ikey == SDLK_KP_7) strcat(szTemp, "numpad_7 ");
        if (ikey == SDLK_KP_8) strcat(szTemp, "numpad_8 ");
        if (ikey == SDLK_KP_9) strcat(szTemp, "numpad_9 ");
        if (ikey == SDLK_KP_MINUS) strcat(szTemp, "- ");
        if (ikey == SDLK_KP_4) strcat(szTemp, "numpad_4 ");
        if (ikey == SDLK_KP_5) strcat(szTemp, "numpad_5 ");
        if (ikey == SDLK_KP_6) strcat(szTemp, "numpad_6 ");
        if (ikey == SDLK_KP_PLUS) strcat(szTemp, "+ ");
        if (ikey == SDLK_KP_1) strcat(szTemp, "numpad_1 ");
        if (ikey == SDLK_KP_2) strcat(szTemp, "numpad_2 ");
        if (ikey == SDLK_KP_3) strcat(szTemp, "numpad_3 ");
        if (ikey == SDLK_KP_0) strcat(szTemp, "numpad_0 ");
        ...
        if (ikey == SDLK_LGUI) strcat(szTemp, "l_windows ");
        if (ikey == SDLK_RGUI) strcat(szTemp, "r_windows ");
        if (ikey == SDLK_POWER) strcat(szTemp, "power ");
        if (strlen(KeyMap[(SDL_Keycode)ikey])) {
            strcat(szTemp, KeyMap[(SDL_Keycode)ikey]);
            strcat(szTemp, "\n");
            fputs(szTemp, fout);
            LogEntry(va("%d %s\n", ikey, szTemp));
        }
...
void C_GUI::drawStatBar(int iX, int iY, int iW, int iT, int iV) {
    ...
    pGFX->DrawBar(iX - 1, iY - 1, iX + iW + 1, iY + 4, dlcsm_LONGRGB(0, 0, 0), dlcsm_LONGRGB(0, 0, 0));
    pGFX->DrawBar(iX, iY, iX + iW, iY + 3, dlcsm_LONGRGB(0, 0, 100), dlcsm_LONGRGB(0, 0, 100));
    float fPercent = (((float)iV / (float)iT) * iW);
    pGFX->DrawBar(iX, iY, iX + (int)fPercent, iY + 3, dlcsm_LONGRGB(0, 100, 0), dlcsm_LONGRGB(0, 100, 0));
}
...
void C_GUI::clearKeys(void) { KeyMap.clear(); }
