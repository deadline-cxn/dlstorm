    ...
    if(visible){
        ...
        switch(type){
            ...
            case FM_GC_DROPBOX:
                    ...
                    if(listoffset < 0) listoffset = 0; x=x+1; y=y+1;
                        ...
                        if(parent_stump->get_control(va("%s_scrollbar",name))){
                            ...
                            while( j< (listdepth+1) ){
                                ...
                                if(pGUI->pMouse->In(x-1,y+(j*16),w+1,y+(j*16)+16)){
                                    ...
                                    r2=pGFX->GetFade(1); g2=pGFX->GetFade(2); b2=pGFX->GetFade(3);
...
void C_GSTMP::mod_control(const char *name,int type,int x,int y,int x2,int y2,int props,char *media,char *value){
    ...
    ngc->rect.left=x;
    ngc->rect.top=y;
    if(type==FM_GC_TICKBOX)   { x2=16; y2=16; ngc->set_value("off"); }
    if(type==FM_GC_GROUPTICK) { x2=16; y2=16; ngc->set_value("off"); }
    if(x2==0) x2=100;
    if(y2==0) y2=15;
    ngc->rect.right=x2;
    ngc->rect.bottom=y2;
...
void C_GUI::checkMouseClicks(void){
    ...
    C_GCTRL *tctrl2=0;
    char temp[1024]; memset(temp,0,1024);
    char console[1024]; memset(console,0,1024);
...
void C_GUI::drawPrompt(void){
    if(bClosePrompt==true) { memset(szPromptMsg,0,1024); del_stump("prompt.gui"); return; }
    if(get_stump("prompt.gui")) { focus_stump=get_stump("prompt.gui"); return; }
...
int  C_GUI::processKeyboard(){
    ...
    if(pMouse->bRightDown) {
        ...
        if(pGFX->pCamera->lastx > pMouse->ix)
            pGFX->pCamera->xrot++;

        if(pGFX->pCamera->lasty > pMouse->iy)
            pGFX->pCamera->yrot++;
        ...
        //if( ( pGFX->pCamera->lastx != pMouse->ix) ||
          //  ( pGFX->pCamera->lasty != pMouse->iy) )
            //    SDL_WarpMouse(pGFX->pCamera->lastx,pGFX->pCamera->lasty);

        pGFX->pCamera->lastx=pMouse->ix;
        pGFX->pCamera->lasty=pMouse->iy;
        */
    }
    else {
