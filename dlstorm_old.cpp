...
bool DLCODESTORM::dlcs_dir_to_file(char const *szDir, char const *szFile, char const *szWildCard) {
    ...
    if (dpdf != NULL) {
        while ((epdf = readdir(dpdf))) {
            if (!((dlcs_strcasecmp(epdf->d_name, ".")) || (dlcs_strcasecmp(epdf->d_name, ".."))))
                if (!dlcs_isdir(epdf->d_name)) fputs(epdf->d_name, fp);
        }
    }
    closedir(dpdf);
    fclose(fp);
...
char const *DLCODESTORM::dlcs_readable_timestamp(char const *x, char const *in) {
    ...
    // long wtf;
    // wtf=atoi(in);
    tTime = (time_t)atoi(in);
    strcpy((char *)x, ctime(&tTime));
    return x;
}
////////////////////////////////////////////////////////////////////////////////////////////////
...
string DLCODESTORM::dlcs_get_os_version() {
        ...
        case VER_PLATFORM_WIN32_NT:
            ...
            if (osvi.dwMajorVersion == 6 && osvi.dwMinorVersion == 0) {
                if (osvi.wProductType == VER_NT_WORKSTATION)
                    strcpy(szTemp, "Windows Server 2008 ");
                else
                    strcpy(szTemp, "Windows Vista ");
            }
            if (osvi.dwMajorVersion == 6 && osvi.dwMinorVersion == 1) {
                if (osvi.wProductType == VER_NT_WORKSTATION)
                    strcpy(szTemp, "Windows Server 2008 R2 ");
                else
                    strcpy(szTemp, "Windows 7 ");
            }
            if (osvi.dwMajorVersion == 6 && osvi.dwMinorVersion == 2) {
                if (osvi.wProductType == VER_NT_WORKSTATION)
                    strcpy(szTemp, "Windows 8 ");
                else
                    strcpy(szTemp, "Windows Server 2012 ");
            }

            sprintf(szTemp2, "version %d.%d %s (Build %d)", (int)osvi.dwMajorVersion, (int)osvi.dwMinorVersion, osvi.szCSDVersion, (int)osvi.dwBuildNumber & 0xFFFF);
 ...
 PGPI pGPI = (PGPI) GetProcAddress(
    ...
    if (z == -1) {
        ...
        x.assign(va("%s %s %s", l_un.sysname, l_un.release, l_un.version));
#endif
    return x;
}
...
string DLCODESTORM::dlcs_get_ipaddress() {
    string x;
    char   y[FILENAME_SIZE];
    memset(y, 0, FILENAME_SIZE);
...
string DLCODESTORM::dlcs_get_webpage(string url) {
    ...
    if ((strx[0] != "http:") && (strx[0] != "https:")) {
        ...
        host = strx[2];
        for (int i = 3; i < abs((int)strx.size()); i++) page = page + "/" + strx[i];
    }
    uri = host + page;
...
////////////////////////////////////////////////////////////////////////////////////////////////
string DLCODESTORM::dlcs_inet_ntoa(string ip) { return ip; }
////////////////////////////////////////////////////////////////////////////////////////////////
