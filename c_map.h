...
public:
    ...
    int         iY;                  // Y coord
    int         iZ;                  // Z coord
    char        szDescription[256];  // Description of item, also used for sign information.    
    RECT        rcMouse;
    CMapObject  *pNext;
...
public:
	...
	u_short GetObj(int x, int y, int priority); // Returns (256*bank+object) - You MUST do a bit AND then divide by 256 to get the bank, to get the object, AND the LSB
	u_short GetObj(int dt, int priority); // Returns (256*bank+object) - You MUST do a bit AND then divide by 256 to get the bank, to get the object, AND the LSB
	void	SetObj(int x,int y, u_char bank, u_char object, u_char priority); // Sets object to bank, object
    void    SetObj(int dt, u_char bank, u_char object, u_char priority);
